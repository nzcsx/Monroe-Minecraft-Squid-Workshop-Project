summon minecraft:armor_stand ~ ~ ~ {CustomName:'"ring_ccw_holder"', CustomNameVisible:0,NoGravity:1,Marker:0,Invisible:1,Tags:["ring_ccw_holder"]}
execute store result score x_holder monroe run data get entity @e[tag=ring_ccw_holder,limit=1] Pos[0] 10000
execute store result score y_holder monroe run data get entity @e[tag=ring_ccw_holder,limit=1] Pos[1] 10000
kill @e[tag=ring_ccw_holder]
